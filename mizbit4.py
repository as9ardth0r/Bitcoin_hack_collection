...
def seek():
    ...
    while True:
        try:
            ...
            contents4 = urllib.request.urlopen("https://blockchain.info/q/getreceivedbyaddress/" + multisig.address).read()
            if int(contents1) > 0 or int(contents2) > 0 or int(contents3) > 0 or int(contents4) > 0:
                ...
                print (colour_yellow + 'Congraz you have found wallet with a balance : ' + colour_reset + multisig.address + colour_green + ' : Balance : ' + str(contents4.decode('UTF8')) + colour_reset)
                ...
                f.write('\nBitcoin Multisig Segwit Address: ' + multisig.address)
                f.write('\nPrivateKey (wif) Compressed : ' + wif2)
                f.write('\nPrivateKey (wif) UnCompressed : ' + wif)
                f.write('\nPublicKey1&2 : ' + cpub + '  &  ' + upub)
                f.write('\n==================Bitcoin Address with Total Received Ammount==========================')
                f.close()
                continue
                ...
                print(colour_green + "Multisig Segwit Address BTC:  " + multisig.address + colour_yellow + ' : ' + key.to_hex() + ' : ' + colour_red + str(contents4.decode('UTF8')) + colour_reset)
                print (colour_green +  ' <================================= Bitcoin Addresses Checked for Total Received Ammount =================================>' + '\n' +  colour_reset)
